...
resource "aws_ssm_parameter" "cis_amazon_linux_2_benchmark_level_2" {
  name  = "/cis_ami/${var.name}/CIS_Amazon_Linux_2_Benchmark_Level_2/ami_id"
  type  = "String"
  value = "placeholder"
...
resource "null_resource" "create_hardened_ami_level_1" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "sudo chmod 755 /usr/bin/kubelet" >> templates/al2/provisioners/install-worker.sh
...
resource "null_resource" "create_hardened_ami_level_2" {
  depends_on = [module.vpc, aws_ssm_parameter.cis_amazon_linux_2_benchmark_level_2]
...
data "aws_ssm_parameter" "cis_amazon_linux_2_benchmark_level_2" {
  depends_on = [aws_ssm_parameter.cis_amazon_linux_2_benchmark_level_2, null_resource.create_hardened_ami_level_2]
  name = "/cis_ami/${var.name}/CIS_Amazon_Linux_2_Benchmark_Level_2/ami_id"
}
...
resource "null_resource" "only_create_hardened_ami_level_2" {
  depends_on = [aws_ssm_parameter.cis_amazon_linux_2_benchmark_level_2]
