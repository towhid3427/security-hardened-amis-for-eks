...
# data "amazon-parameterstore" "eks_optimized_ami_al2" {
#   name   = "/aws/service/eks/optimized-ami/${var.eks_version}/${var.ami_type}/recommended/image_id"
#   region = var.aws_region
# }

data "amazon-ami" "eks_optimized_ami_al2" {
    filters = {
        virtualization-type = "hvm"
        name = "amazon-eks-node-1.32-v20250904" ## Change the AMI Here To Use a Different Source AMI.
...
source "amazon-ebs" "eks_optimized_ami_al2" {
  ...
  source_ami              = data.amazon-ami.eks_optimized_ami_al2.id
...
build {
  name    = "eks_optimized_ami_al2"
  sources = ["source.amazon-ebs.eks_optimized_ami_al2"]

provisioner "shell-local" {
    command = "tar cf toupload/files.tar --no-xattrs -C .. cis-scripts"
}
...
provisioner "shell" {
  inline = [
    try(var.shell_provisioner1, null)
  ]
}

provisioner "file" {
    source      = try(var.file_provisioner1, null)
    direction   = "download"
...
post-processor "manifest" {
    ...
    custom_data = {
      ...
      hardened_ami_name = "${local.ami_name}"
}  
