...
resource "aws_ssm_parameter" "eks_optimized_al2_level_1" {
  name  = "/cis_ami/${var.name}/EKS_Optimized_al2_Level_1/ami_id"
  type  = "String"
  value = "placeholder"
...
resource "aws_ssm_parameter" "eks_optimized_al2_level_2" {
  name  = "/cis_ami/${var.name}/EKS_Optimized_al2_Level_2/ami_id"
  type  = "String"
  value = "placeholder"
...
resource "null_resource" "create_hardened_ami_level_1" {
  depends_on = [module.vpc, aws_ssm_parameter.eks_optimized_al2_level_1, aws_ssm_parameter.eks_optimized_al2_level_2]
  triggers = {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      cd packer-files && \
      packer init -upgrade . && \
      packer build -only 'eks_optimized_ami_al2.*' \
        -var-file=al2_amd64_level_1.pkrvars.hcl \
      ...
      --name "/cis_ami/EKS_Optimized_AL2/EKS_Optimized_al2_Level_1/ami_id" \
      --type "String" \
...
################################################################################
# Create Hardened AMI EKS_Optimized_AL2_Level_2
################################################################################
...
resource "null_resource" "create_hardened_ami_level_2" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      packer init -upgrade . && \
      packer build -only 'eks_optimized_ami_al2.*' \
        -var-file=al2_amd64_level_2.pkrvars.hcl \
        -var "subnet_id=${module.vpc.public_subnets[0]}" \
        -var "aws_region=${var.aws_region}" \
        . && \
      ...
      echo $AMI_ID_Level_2 && \
      aws ssm put-parameter \
      --name "/cis_ami/EKS_Optimized_AL2/EKS_Optimized_al2_Level_2/ami_id" \
      --type "String" \
...
data "aws_ssm_parameter" "eks_optimized_al2_level_1" {
  depends_on = [aws_ssm_parameter.eks_optimized_al2_level_1, null_resource.create_hardened_ami_level_1]
  name = "/cis_ami/${var.name}/EKS_Optimized_al2_Level_1/ami_id"
}
...
module "eks_managed_node_group_level_1" {
  ...
  ami_id                            = data.aws_ssm_parameter.eks_optimized_al2_level_1.value
  ami_type                          = "AL2_x86_64"
...
################################################################################
# EKS Managed Node Group Modules For AL2 CIS Level 2
################################################################################

data "aws_ssm_parameter" "eks_optimized_al2_level_2" {
  depends_on = [aws_ssm_parameter.eks_optimized_al2_level_2, null_resource.create_hardened_ami_level_2]
  name = "/cis_ami/${var.name}/EKS_Optimized_al2_Level_2/ami_id"
}
...
module "eks_managed_node_group_level_2" {
  ...
  vpc_security_group_ids            = [module.eks_cluster.node_security_group_id]
  cluster_service_cidr              = module.eks_cluster.cluster_service_cidr
  ami_id                            = data.aws_ssm_parameter.eks_optimized_al2_level_2.value
  ami_type                          = "AL2_x86_64"
  cluster_endpoint                  = module.eks_cluster.cluster_endpoint
  cluster_auth_base64               = module.eks_cluster.cluster_certificate_authority_data
}
...
################################################################################
# Create Hardened AMI EKS_Optimized_AL2_Level_2 Only
################################################################################
